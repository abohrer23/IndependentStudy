...
public class Main {
	...
	public void makeboard(int quantity, int level) {
		...
		//Get Batch #
		try {
			Scanner scanner = new Scanner(new File("boardconfig"));
			...
			try {
				FileWriter myWriter = new FileWriter("boardconfig");
	...
	private void writefile(int[][] board, int batch){
		...
		//File I/O code obtained from: https://www.w3schools.com/java/java_files_create.asp
		try {
			String destination = "../genboards/outputs/gennedboard-" + batch + "-" + this.sample + " .txt"; //support_src/boards/resources/gennedboard-" + batch + "-" + this.sample + " .txt";
